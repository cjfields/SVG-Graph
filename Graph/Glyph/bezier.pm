...
sub draw {
    ...
    foreach my $i ( 1 .. $#p ) {
        ...
        for ( my $d = 2; $d <= 100; $d += 2 ) {
            ...
            my ( $int, $t1, $t2 ) = Intersection(
                Segment( [ $x1,          $y1 ],  [ $x1 + $delta, $y1d ] ),
                Segment( [ $x2 - $delta, $y2d ], [ $x2,          $y2 ] )
            );
            next unless ref($int);
        ...
        if ($e) {
            ...
            $cy = ( $self->ysize - ( $cy - $self->group->ymin ) * $yscale )
                + $self->yoffset;
...
sub spline_generate {
    ...
    for ( $i = @points - 2; $i >= 0; $i-- ) {
        $coeffs[$i] = ( $coeffs[$i] || 0 ) * ( $coeffs[ $i + 1 ] || 0 )
            + ( $factors[$i] || 0 );
    }
    return \@coeffs;
}
