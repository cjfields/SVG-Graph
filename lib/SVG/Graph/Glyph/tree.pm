...
sub draw {
    ...
    $group->line(
        ...
        x2 => $xoffset + ( $xsize / 2 ),
        y1 => $yoffset,
        y2 => $yoffset + ( $yscale * $root->branch_length ),
        style => { $self->_style }
    );

    my $maxy = $self->getmaxy(
        node    => $root,
        yscale  => $yscale,
        yoffset => $yoffset + ( $yscale * $root->branch_length )
    );
...
sub _draw {
    ...
    foreach my $d ( $a{node}->daughters ) {
        ...
        if ( $d->depth_under <= 1 ) {
            $a{group}->line(
                x1 => ( ( $frac * $a{xsize} + $xoffset ) + ($xoffset) ) / 2,
                x2 => ( ( $frac * $a{xsize} + $xoffset ) + ($xoffset) ) / 2,
                y1    => $a{yoffset},
                y2    => $maxy,
                style => {%style}
            );
        ...
        #root branch label
        my ( $cx, $cy )
            = ( ( ( $frac * $a{xsize} + $xoffset ) + ($xoffset) ) / 2,
            $maxy );
